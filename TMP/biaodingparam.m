...
% 1. 真实参数（用于生成仿真数据）
% -----------------------------
phi_true = [-2e-4, 2e-5, -2e-5];     % rad
psi_true = [-0.9e-3, 1.1e-3, -0.6e-3];    % rad

a_true = [ -386.5e-3,  157.7e-3;   % a0x, a0y
           -53.2e-3, -241e-3;   % a1x, a1y
            53.8e-3, -241e-3 ]; % a2x, a2y   (单位: m)

b_true = [213e-3; 142e-3; 142e-3];         % m
lambda_true = 1;

global l_true;
l_true = zeros(3,1);
for i = 1:3
    gamma = psi_true(i) - phi_true(i);
    f = -cos(gamma) / cos(2*gamma);
    
    if i == 1  % i=0 in math, but MATLAB index starts at 1
        zeta = psi_true(1) - pi/2;
    else
        zeta = psi_true(i);
    end
    
    g = b_true(i) - a_true(i,1)*sin(zeta) + a_true(i,2)*cos(zeta);
    l_true(i) = f * g;
end
l_true = [173.5e-3, 99e-3, 99e-3];

% 打包真实参数向量 p = [phi0,phi1,phi2, psi0,psi1,psi2, a0x,a0y,a1x,a1y,a2x,a2y, b0,b1,b2]
p_true = [phi_true, psi_true, a_true(:)', b_true', lambda_true];%, l_true'];
...
% x: -1mm ~ +1mm
t = (0:N-1) * Ts;
x_data = 1e-3 * cos(2*pi*2*t);  
% y: -0.5cm ~ 23cm → -0.005m ~ 0.23m
y_data = 1e-1 + 1e-1 * sin(2*pi*0.5*t);
% theta: -2mrad ~ +2mrad
theta_data = 1e-3 * sin(2*pi*4*t);
...
l0_guess = l_true;% + 1e-3;

p0 = [phi0_guess, psi0_guess, a0_guess(:)', b0_guess, lambda0_guess];%, l0_guess'];  % 全是 1x?，可拼接
...
param_names = {'phi0','phi1','phi2','psi0','psi1','psi2',...
               'a0x','a0y','a1x','a1y','a2x','a2y','b0','b1','b2','lambda', 'l0','l1','l2'};
...
function h = sensor_model(x, y, theta, p)
    ...
    %     end
    % 
    %     g = b(i) - a(i,1)*sin(zeta) + a(i,2)*cos(zeta);
    %     l_true(i) = f * g;
    % end
