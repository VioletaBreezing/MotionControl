...
h_(1) = -cal_eddy_dis(R, t, a0, u0, p0_B, n0_B, l0(1));
h_(2) = cal_eddy_dis(R, t, a1, u1, p1_B, n1_B, l0(2));
h_(3) = cal_eddy_dis(R, t, a2, u2, p2_B, n2_B, l0(3));
...
y_true  = 2e-3 * cos(2*pi*t);
rz_true = 2e-3 * sin(4*pi*t);
...
param.a{1} = a0; param.a{2} = a1; param.a{3} = a2;
param.u{1} = u0; param.u{2} = u1; param.u{3} = u2;
param.p{1} = p0_B; param.p{2} = p1_B; param.p{3} = p2_B;
param.n{1} = n0_B; param.n{2} = n1_B; param.n{3} = n2_B;
param.l0 = l0;

param_cal.ax = ax;
param_cal.ay1 = ay1; param_cal.ay2 = ay2;
param_cal.bx = bx; param_cal.by = by;
...
max_err = [norm(err_x, inf), norm(err_y, inf), norm(err_rz, inf)];
...
function l = cal_eddy_len(R, t, a, u, p, n);%, l0)
    N = R * n;
    P = R * p + t;
    beta = dot(u, N);
    l = dot((P - a), N) / beta;
